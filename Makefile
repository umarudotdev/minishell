NAME		:= $(shell grep -oP 'MINISHELL_NAME "\K[^"]+' include/minishell.h)

VERSION		:= $(shell grep -oP 'MINISHELL_VERSION "\K[^"]+' include/minishell.h)
...
$(NAME): $(LIBS) $(OBJS)
	$(CC) $(LDFLAGS) $(OBJS) $(LDLIBS) -o $(BUILD_DIR)/$(NAME)
